...
public class PublicApiService implements IPublicApi{
    ...
    public Result login(Request<User> request) throws TipsException {
        ...
        redisUtils.set(loginUser.getId(), token, Constant.LOGIN_EXPIRETIME);
        redisUtils.set(secret, loginUser, Constant.LOGIN_EXPIRETIME);
    ...
    public Result getAuths(Request<User> request) {
        ...
        User authorsUser = (User) redisUtils.get(secret);
        if(authorsUser == null){
